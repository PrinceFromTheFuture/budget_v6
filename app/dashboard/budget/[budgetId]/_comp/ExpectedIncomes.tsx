...
export function ExpectedExpensess({ categories, budget }: { categories: Category[]; budget: Budget }) {
  ...
  const chartData = budget.budgetCategories
    .filter((cat) => cat.type === "income")
    .map((category, i) => {
      const cat = categories.find((cat) => cat.id === category.budgetCategoryId)!;
      return { browser: cat.name, visitors: Number(category.amountAllocated), fill: colors[i] };
    });
  ...
  return (
    <Card className="flex w-full  m-0 flex-col">
      <CardHeader className="items-center pb-0">
        <CardTitle>Budget expeted Incomes</CardTitle>
        <CardDescription>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        ...
        <div className="leading-none text-muted-foreground">How much you need to Earn a day</div>
      </CardFooter>
    </Card>
  );
}
